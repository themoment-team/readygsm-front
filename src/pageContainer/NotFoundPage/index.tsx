...
import { Arrow, Four, Zero } from '@/assets';
...
const NotFoundPage = () => {
  ...
  return (
    ...
    >
      <div className={cn('relative flex flex-col items-center justify-center')}>
        ...
        <div className={cn('absolute left-[20rem] top-[90%] flex flex-row gap-[0.97rem]')}>
          {squares.map((square, index) => (
            <div
              key={index}
              className={cn('square.bgColor square.rotate h-[0.9375rem] w-[0.9375rem] rounded-md')}
            />
          ))}
        </div>
      </div>
      ...
      <p
        className={cn(
          'mt-10 text-[1.5rem] font-semibold text-black mobile:text-[1.125rem] mobile-lg:text-[1.25rem] tablet-sm:text-[1.375rem] tablet:text-[1.875rem]',
        )}
      >
        페이지를 찾을 수 없음
      </p>
      ...
      <p
        className={cn(
          'mt-2 text-[1rem] text-gray-500 tablet-sm:text-[1.125rem] tablet:text-[1.25rem]',
        )}
      >
      ...
      <button
        onClick={() => GoBack()}
        className={cn(
          'mt-8 flex items-center space-x-2 rounded-full border border-gray-400 bg-white px-6 py-2 text-gray-500 transition hover:bg-gray-100',
        )}
      >
        <Arrow />
